...
<!--<script>
Push.create("Hola bienvenido al foro!", {
  body: "Te animas a crear un tema amigo?",
  icon: 'cuerpo/vista/app/images/images/notificacion/notificacion.gif',
  timeout: 10000,
  onClick: function () {
      window.focus();
      this.close();
    ...
    if($prepare_sql->num_rows > 0){
        ...
        while($prepare_sql->fetch()){//la cantidad de veces tanto foros como haya mostraremos el echo siguiente
          ...
          echo '<div class="row foros">
            ...
            <div class="col-md-7 col-sm-7 col-xs-4 puntitos" style="padding-left: 30px;">
              <a href="foros/'.UrlAmigable($id_del_foro,$foros[$id_del_foro]['nombre']).'">'.$foros[$id_del_foro]['nombre'].'</a><br />
              '.$foros[$id_del_foro]['descrip'].'
            </div>
            <div class="col-md-2 col-sm-2 col-xs-4 left_border" style="text-align: center;font-weight: bold;">
              '.number_format($foros[$id_del_foro]['cantidad_temas'],0,',','.').'Temas<br />
              '.number_format($foros[$id_del_foro]['cantidad_mensajes'],0,',','.').' Mensajes
            </div>
            <div class="col-md-2 col-sm-2 col-xs-2 left_border puntitos" style="line-height: 37px;">
              '.$ultimo_tema.'
            </div>
          </div>';
        }
