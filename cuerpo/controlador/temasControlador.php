...
if(isset($_GET['id_foro']) and array_key_exists($_GET['id_foro'],$foros)){
    $id_foro = intval($_GET['id_foro']);
    require('cuerpo/modelo/class.Temas.php');
    $isset_id = isset($_GET['id']) and is_numeric($_GET['id']) and $_GET['id'] >= 1;
    $loged = isset($_SESSION['app_id']);
    if($loged){
      $cerrado = $foros[$id_foro]['estado'] == 1 or $users[$_SESSION['app_id']]['rol'] == 2;
    }else{
      $cerrafo = false;
    }
    $temas = new Temas();
